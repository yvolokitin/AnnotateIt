...
class AppLocalizationsRu extends AppLocalizations {
  ...
  String get modelsHelpTips =>
      'Совет:\n• Лучшие подсказки: начните с рамки или 1–2 положительных кликов, затем добавьте отрицательные клики, чтобы исключить лишнее.\n• Уточняйте, а не рисуйте заново: добавляйте новые подсказки, чтобы обновить ту же маску.\n• Производительность: энкодер — самый тяжёлый шаг; используйте его повторно для множества подсказок на одном изображении.\n• Большие изображения: увеличьте масштаб перед подсказками для более чётких границ.\n• Квантование vs полная точность: квантованные модели работают быстрее и используют меньше памяти, но могут терять мелкие детали.';

  @override
  String get aboutTitle => 'О приложении AnnotateIt';

  @override
  String get aboutDescription =>
      'AnnotateIt — это приложение для разметки, разработанное для оптимизации процесса аннотирования в проектах компьютерного зрения. Независимо от того, работаете ли вы над классификацией изображений, обнаружением объектов, сегментацией или другими задачами, AnnotateIt обеспечивает гибкость и точность, необходимые для качественной разметки данных.';
  ...
  String get aboutCallToAction =>
      'Начните работу с AnnotateIt уже сегодня и создавайте качественные датасеты для моделей компьютерного зрения!';
  ...
  @override
  String get betaDialogTitle => 'Модели сегментации (Preview)';

  @override
  String get betaDialogMessage =>
      'Почему SAM Mobile, SAM2 Hiera Base+ и SAM2 Hiera Large часто дают похожие результаты?\n\nНе‑веб сборки (Windows/macOS/Linux/Android/iOS): реальный вывод SAM пока не включён. Приложение использует эвристику, зависящую от содержимого (рост области от семени + извлечение контуров). Смена модели не меняет эту эвристику, поэтому результаты схожи.\n\nВеб‑сборки: используется реальная ONNX‑инференция, когда модель успешно загружается, но результаты всё равно постобрабатываются одинаково: маска декодера 256×256 → порогование (Отсу), выбор компонента, связанного с точкой клика, заполнение отверстий, сглаживание и упрощение полигона. Это может убирать мелкие детали и визуально сближать модели больше, чем ожидается.\n\nИзвестные ограничения предпросмотра:\n • Только одноточечные подсказки (без рамок/многоточечных подсказок).\n • Изображение приводится к 1024 px, маска декодера 256×256 → потеря деталей на крошечных объектах/границах.\n • Порогование/сглаживание может пере‑/недосегментировать в зависимости от контраста/шума.\n • На не‑веб платформах переключение модели влияет в основном на подписи/интерфейс, а не на сам бэкенд сегментации.';

  @override
  String get betaDialogTips =>
      'Советы, как улучшить результаты уже сейчас:\n • Приблизьте и кликайте значительно внутри нужного объекта.\n • Предпочитайте изображения с хорошим локальным контрастом и минимальными артефактами сжатия.\n • По возможности попробуйте веб‑версию для реальной инференции SAM.\n • При необходимости вручную подправляйте границы после авто‑сегментации.';
  ...
  String get removeFilesDbOnlyNote =>
      'Примечание: Будут удалены только записи в датасете. Оригинальные файлы на диске не будут удалены этой операцией. Однако если вы видите отсутствующие изображения или ошибки (например, ErrorImageTile), возможно, файл уже был удалён с диска или у вас нет прав доступа к нему.';
